...
for i in 0:numExposures-1

    print('Adding picture %i of %i \n', i, numExposures);

    image = double(imread(dir_name + filenames[0]));
...
% Don
't multiply with the weights since they are zero for saturated
% pixels.m
contains
the
logRadiance
...
the
sum
for those pixels to avoid division by zero
